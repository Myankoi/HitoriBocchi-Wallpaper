jQuery(document).ready(function ($) {
  ...
  var bg_star_created = 0,
    obj_bg_star = [],
    n_bg_star_num = 0;
  var bocchi_effect_created = 0,
    obj_effect_star = [],
    n_effect_star_num = 0;
  var loading = 0;
  var step_fps = 50;
  var bocchi_interaction_activated = 0,
    background_change = 0;
  ...
  function automatic_loading() {
    ...
    c_w = 1920 / view_width;
    document.getElementById("bg_night").style.background =
      "radial-gradient(#1f252c,#0b0d0f)";
  ...
  function falling_star() {
    ...
    star_imgs_obj.style.transform =
      "rotate(" + (irandom_range(0, 90) - 45) + "deg)";
  ...
  function interacting_bocchi() {
    ...
    if (imgs_obj.style.top != view_height + 320 + "px") {
      ...
      for (var i = 0; i < n_effect_star_num - 1; i++) {
        obj_effect_star[i].style.top =
          -1280 +
          parseInt(window.getComputedStyle(obj_effect_star[i]).top) +
          "px";
      }
  ...
  function StepEvent() {
    ...
    if (automatic_cleaning >= 0 && bocchi_effect_created > 0) {
      ...
      if (obj_effect_star[__ii]) {
        obj_effect_star[__ii].style.top =
          -320 -
          bocchi_interaction_activated * 1280 +
          parseInt(window.getComputedStyle(obj_effect_star[__ii]).top) +
          "px";
    ...
    if (
      ...
      bocchi_yy < view_height &&
      random_val <= 2000 / step_fps
    ) {
      ...
      obj_effect_star[n_effect_star_num].style.transition =
        "all " + transition_time + "s";
    ...
    if (automatic_cleaning >= 0 && bg_star_created == 1) {
      if (obj_bg_star[n_bg_star_num - 1]) {
        var current_y = parseInt(
          window.getComputedStyle(obj_bg_star[n_bg_star_num - 1]).top
        );
        obj_bg_star[n_bg_star_num - 1].style.top =
          current_y - view_height - 64 + "px";
    ...
    if (
      automatic_cleaning >= 0 &&
      bocchi_yy < view_height * 0.7 &&
      (random_val <= 150 / step_fps || redecorate == 1)
    ) {
      ...
      obj_bg_star[n_bg_star_num].style.transition =
        "top " +
        transition_time +
        "s, opacity " +
        (3 + floor(distance__ / 500)) +
        "s";
